...
//        std::cerr << std::endl;
        ...
        for (int doc = 0; doc < numDocs; ++doc)
        {
            ...
            target[doc] = learnTarget[docIdx]; //[permutedDocIdx];
            weight[doc] = learnWeight[docIdx]; //[permutedDocIdx];

            for (int dim = 0; dim < approxDimension; ++dim)
                approx[dim][doc] = avrgApprox[dim][docIdx]; //[permutedDocIdx];
                 ...
                 TLearnContext* ctx)
{
    ...
    double learningRate = ctx->Params.BoostingOptions->LearningRate.Get();
    ...
    for (int dim = 0; dim < approxDimension; ++dim) {
            ...
            if (var > 0 && var > 4 * avgVar[dim]) {
                double smooth = std::min(learningRate, sqrt(avgVar[dim] / var));
